...
/*** DEFINES                  ***/
#define REGISTER_PLUGIN_FUNCTION_PRIV_NAME      NewLineProcessorCR // The name to append on the RegisterPlugin() function for built in version
...
/*** FUNCTION PROTOTYPES      ***/
const struct DataProcessorInfo *NewLineProcessorCR_GetProcessorInfo(
        unsigned int *SizeOfInfo);
void NewLineProcessorCR_ProcessByte(t_DataProcessorHandleType *DataHandle,
        const uint8_t RawByte,uint8_t *ProcessedChar,int *CharLen,
        PG_BOOL *Consumed);

/*** VARIABLE DEFINITIONS     ***/
struct DataProcessorAPI m_NewLineProcessorCRCBs=
{
    NULL,
    NULL,
    NewLineProcessorCR_GetProcessorInfo,
    NULL,
    NewLineProcessorCR_ProcessByte,
};
struct DataProcessorInfo m_NewLineProcessorCR_Info=
{
    "New Line Processor CR",
    "Added a '\\n' when it sees a '\\r'",
    "Does a new line when a cartridge return character is read",
...
/*******************************************************************************
 * NAME:
 *    NewLineProcessorCR_RegisterPlugin
 *
 * SYNOPSIS:
 *    unsigned int NewLineProcessorCR_RegisterPlugin(const struct PI_SystemAPI *SysAPI,
...
extern "C"
{
            ...
            unsigned int Version)
    {
        ...
        m_NLP_DPS->RegisterDataProcessor("NewLineProcessorCR",
                &m_NewLineProcessorCRCBs,sizeof(m_NewLineProcessorCRCBs));
        ...
        unsigned int *SizeOfInfo)
{
    *SizeOfInfo=sizeof(struct DataProcessorInfo);
    return &m_NewLineProcessorCR_Info;
}
